   ...
   /**
    ...
    class DesktopMenu extends JPopupMenu
    {
        ...
        public DesktopMenu(ActionListener listener)
        {
            ...
            create_shortcut = add(WorkspaceUtils.createMenuItem
                                  (listener, LangResource.getString("Desktop.menu.createShortcut") + "...",
                                   Desktop.CREATE_SHORTCUT, null));

            paste = add(WorkspaceUtils.createMenuItem
            ...
            select_all = add(WorkspaceUtils.createMenuItem
    ...
    private Point getTransferVector(int xshift, int yshift)
    {
        ...
        for (int i = 0; i < desktopIcons.size(); i++)
        {
            ...
            if (icon.isSelected())
            {
                ...
                topLeft.x = WorkspaceUtils.min(topLeft.x, location.x +
                                                          getParent().getLocation().x + getLocation().x);
                topLeft.y = WorkspaceUtils.min(topLeft.y, location.y +
                                                          getParent().getLocation().y + getLocation().y);

                bottomRight.x = WorkspaceUtils.max(bottomRight.x, location.x +
                                                                  getParent().getLocation().x + getLocation().x + icon.getWidth());
                bottomRight.y = WorkspaceUtils.max(bottomRight.y, location.y +
        ...
        Rectangle groupBounds = new Rectangle(topLeft.x, topLeft.y,
                                              WorkspaceUtils.distance(topLeft.x, bottomRight.x),
                                              WorkspaceUtils.distance(topLeft.y, bottomRight.y));
    ...
    private Point getTransferVector(int finalx, int finaly, DesktopIconSelectionData icons)
    {
        ...
        for (int i = 0; i < icons.getSize().intValue(); i++)
        {
            ...
            topLeft.x = WorkspaceUtils.min(topLeft.x, xlocation +
                                                      getParent().getLocation().x + getLocation().x);
            topLeft.y = WorkspaceUtils.min(topLeft.y, ylocation +
                                                      getParent().getLocation().y + getLocation().y);

            bottomRight.x = WorkspaceUtils.max(bottomRight.x, xlocation +
                                                              getParent().getLocation().x + getLocation().x + iconData.getWidth().intValue());
            bottomRight.y = WorkspaceUtils.max(bottomRight.y, ylocation +
    ...
    public boolean isModified()
    {
        ...
        for (int i = 0; i < internalFrames.length; i++)
        {
            try
            {
                Method mod_meth = internalFrames[i].getClass().getMethod("isModified", new Class[]{});
                Object result = mod_meth.invoke(internalFrames[i], new Object[0]);
    ...
    {
        if (isGroupSelected() && icon.isSelected())
            return true;
        return false;
    }
    ...
    public void mouseDragged(MouseEvent e)
    {
        ...
        selectPane.setBounds(WorkspaceUtils.min(startingSelection.x, endingSelection.x),
                             WorkspaceUtils.min(startingSelection.y, endingSelection.y),
                             WorkspaceUtils.distance(endingSelection.x, startingSelection.x),
                             WorkspaceUtils.distance(endingSelection.y, startingSelection.y));
    }
    ...
    {
        Rectangle selector =
                new Rectangle(WorkspaceUtils.min(startingSelection.x, endingSelection.x),
                              WorkspaceUtils.min(startingSelection.y, endingSelection.y),
                              WorkspaceUtils.distance(endingSelection.x, startingSelection.x),
                              WorkspaceUtils.distance(endingSelection.y, startingSelection.y));
    ...
    protected void selectNextIcon(int direction, DesktopIcon cursor_icon)
    {
        ...
        for (int i = 0; i < desktopIcons.size(); i++)
        {
             ...
             */
            if (current_icon.equals(cursor_icon)) continue;
            /**
            ...
            if ((direction == Desktop.ICON_ON_NORTH &&
                    WorkspaceUtils.isNorthernQuadrant(cursor_icon_point, current_icon_point))
                    || (direction == Desktop.ICON_ON_SOUTH &&
                    WorkspaceUtils.isSouthernQuadrant(cursor_icon_point, current_icon_point))
                    || (direction == Desktop.ICON_ON_EAST &&
                    WorkspaceUtils.isEasternQuadrant(cursor_icon_point, current_icon_point))
                    || (direction == Desktop.ICON_ON_WEST &&
                    WorkspaceUtils.isWesternQuadrant(cursor_icon_point, current_icon_point)))
            {
                /**
                ...
                if (WorkspaceUtils.distance(cursor_icon_point, min_icon_point)
                        >= WorkspaceUtils.distance(cursor_icon_point, current_icon_point))
                {
                    min = current_icon;
                    min_icon_point = current_icon_point;
                }
