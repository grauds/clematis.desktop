...
// file dialogs
    ...
    private static DirectorySelectorDialog dirChooser = null;
    ...
    {
        dirChooser = new DirectorySelectorDialog
                (parent,
                 LangResource.getString("Cache.chooseDirectory"),
                 true);

        UIManager.addPropertyChangeListener(new UISwitchListener(dirChooser));
        dirChooser.centerDialog();
        dirChooser.setRoot(root);
    ...
    {
        if (getIconChooser().showOpenDialog(parent)
                != JFileChooser.APPROVE_OPTION)
            return (null);
