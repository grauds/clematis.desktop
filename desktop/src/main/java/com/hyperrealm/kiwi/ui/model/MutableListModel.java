...
public class MutableListModel<T> implements ListModel, Iterable<T> {
    ...
    protected void fireIntervalAdded(int start, int end) {
        ...
        synchronized (listeners) {
            for (ListDataListener l : listeners) {
                if (evt == null) {
                    evt = new ListDataEvent(this, ListDataEvent.INTERVAL_ADDED, start, end);
                }
    ...
    protected void fireIntervalRemoved(int start, int end) {
        ...
        synchronized (listeners) {
            for (ListDataListener l : listeners) {
                if (evt == null) {
                    evt = new ListDataEvent(this, ListDataEvent.INTERVAL_REMOVED, start, end);
                }
    ...
    protected void fireContentsChanged(int start, int end) {
        ...
        synchronized (listeners) {
            for (ListDataListener l : listeners) {
                if (evt == null) {
                    evt = new ListDataEvent(this, ListDataEvent.CONTENTS_CHANGED, start, end);
                }
