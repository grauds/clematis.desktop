...
public class ExternalKTreeModel<T> extends DefaultKTreeModel<T> {
    ...
    public void reload(T node) {
        ...
        if (node == null) {
            ...
            columnNames = (String[]) source.getValueForProperty(null, TreeDataSource.COLUMN_NAMES_PROPERTY);
            columnTypes = (Class[]) source.getValueForProperty(null, TreeDataSource.COLUMN_TYPES_PROPERTY);
    ...
    protected void loadChildren(TreeNode node) {
        ...
        for (T child : children) {
            list.add(makeNode(child, node)); // this is UGLY!
        }
